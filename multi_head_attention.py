...
class MultiHeadAttention(nn.Module):
    ...
    def _att_dot(self, q, k, v):
        #k,v,q:[batch_size, num_heads, seq_len, dim_k/dim_v]
        scores = torch.matmul(q, k.transpose(-2, -1)) / torch.sqrt(torch.tensor(self.dim_k))
        #scores:[batch_size, num_heads, seq_len, seq_len]
        scores =scores.masked_fill(self.mask.bool(), float('-inf'))
        scores=nn.functional.softmax(scores, dim=-1)
        scores = self.dropout_layer(scores)
        att_dot =torch.matmul(scores, v)
        return att_dot
...
def test():   
    ...
    model = MultiHeadAttention(num_heads, dim_k, dim_v, dim_input).to("mps")
    
    batch_size=2
    seq_len=4
    x_input = torch.randn(batch_size, seq_len, dim_input).to("mps")
